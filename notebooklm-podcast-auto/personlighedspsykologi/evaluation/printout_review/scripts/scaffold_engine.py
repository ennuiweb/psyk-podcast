...
PROMPT_VERSION = "personlighedspsykologi-reading-scaffold-v3"
SCHEMA_VERSION = 3
...
def scaffold_generation_config_metadata() -> dict[str, Any]:
...
def _require_text(value: Any, field_name: str) -> str:
    ...
    if not text:
        raise ScaffoldingError(f"scaffold field must not be empty: {field_name}")
    return text
...
def _reject_broad_prompt(text: str, field_name: str) -> None:
    ...
    if any(re.search(pattern, lowered) for pattern in broad_patterns):
        raise ScaffoldingError(f"{field_name} is too broad for a reading micro-task")
...
def _require_safe_task_hint(value: Any, field_name: str, *, forbid_parentheses: bool = False) -> str:
    ...
    if _looks_like_answer_leak(text, forbid_parentheses=forbid_parentheses):
        raise ScaffoldingError(f"{field_name} must not reveal the answer")
    return text
...
def _require_answer_shape(value: Any, field_name: str) -> str:
    ...
    if len(text) > 80:
        raise ScaffoldingError(f"{field_name} must describe answer format only")
    lowered = text.lower()
...
def validate_v2_scaffold_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(guide_overview) != 3:
        raise ScaffoldingError("abridged guide must include a three-sentence overview")
    ...
    if not 3 <= len(structure_map) <= 7:
        raise ScaffoldingError("abridged guide must include 3-7 structure-map items")
    ...
    if not 2 <= len(_coerce_list(guide.get("do_not_get_stuck_on"))) <= 5:
        raise ScaffoldingError("abridged guide must include 2-5 do-not-get-stuck-on items")
    ...
    for item in guide_overview + overview:
        if "____" in str(item):
            raise ScaffoldingError("overview sentences must not contain blanks")
    for index, item in enumerate(structure_map, start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each structure-map item must be an object")
    ...
    for index, item in enumerate(quote_targets, start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each quote target must be an object")
        target = _require_text(item.get("target"), f"abridged_guide.key_quote_targets[{index}].target")
        if len(target) > 140:
            raise ScaffoldingError("quote targets must be short search phrases, not reproduced passages")
    ...
    for index, item in enumerate(questions, start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each unit-test question must be an object")
        ...
        if not question.endswith("?"):
            raise ScaffoldingError("each unit-test question must be phrased as a question")
        if len(question) > 220:
            raise ScaffoldingError("unit-test questions must stay short and concrete")
    ...
    for index, item in enumerate(fill_ins, start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each cloze sentence must be an object")
        ...
        if _blank_count(sentence) != 1:
            raise ScaffoldingError("each cloze sentence must contain exactly one blank marker")
        if len(sentence) > 220:
            raise ScaffoldingError("cloze sentences must stay short enough for print use")
    ...
    for index, item in enumerate(diagrams, start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each diagram task must be an object")
        ...
        if len(elements) < 2:
            raise ScaffoldingError("each diagram task must include at least two required elements")
        for element in elements:
...
def _require_object(value: Any, field_name: str) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise ScaffoldingError(f"scaffold payload missing object: {field_name}")
    return value


def _require_count(items: list[Any], field_name: str, minimum: int, maximum: int) -> None:
    if not minimum <= len(items) <= maximum:
        raise ScaffoldingError(f"{field_name} must include {minimum}-{maximum} items")
...
def _require_short_quote_anchor(value: Any, field_name: str) -> str:
    ...
    if len(text) > 180 or _word_count(text) > 18:
        raise ScaffoldingError(f"{field_name} must be a short quote anchor, not a reproduced passage")
    return text
...
def _validate_v3_reading_guide(guide: dict[str, Any]) -> None:
    ...
    if len(overview) != 3:
        raise ScaffoldingError("reading guide must include a three-sentence overview")
    ...
    for item in overview:
        if "____" in str(item):
            raise ScaffoldingError("reading guide overview must not contain blanks")
    for index, item in enumerate(route, start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each reading-route item must be an object")
    ...
    for index, item in enumerate(quote_targets, start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each reading-guide quote target must be an object")
...
def _validate_v3_abridged_reader(reader: dict[str, Any]) -> None:
    ...
    for index, item in enumerate(sections, start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each abridged-reader section must be an object")
        ...
        if len(quote_anchors) > 3:
            raise ScaffoldingError("each abridged-reader section may include at most 3 quote anchors")
        ...
        for paragraph in paragraphs:
            ...
            if _word_count(text) > 95:
                raise ScaffoldingError("abridged-reader paragraphs must stay short")
        for anchor_index, anchor in enumerate(quote_anchors, start=1):
            if not isinstance(anchor, dict):
                raise ScaffoldingError("each quote anchor must be an object")
        ...
        if not question.endswith("?"):
            raise ScaffoldingError("abridged-reader mini checks must be questions")
...
def _validate_v3_active_reading(active: dict[str, Any]) -> None:
    ...
    for index, item in enumerate(abridged_checks, start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each abridged check must be an object")
        ...
        if not question.endswith("?"):
            raise ScaffoldingError("abridged checks must be phrased as questions")
    ...
    for index, item in enumerate(source_touchpoints, start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each source touchpoint must be an object")
        ...
        if _word_count(task) > 45:
            raise ScaffoldingError("source touchpoint tasks must stay tiny")
        _require_text(
...
def _validate_v3_consolidation(consolidation: dict[str, Any]) -> None:
    ...
    if len(overview) != 3:
        raise ScaffoldingError("consolidation sheet must include a three-sentence overview")
    ...
    for item in overview:
        if "____" in str(item):
            raise ScaffoldingError("consolidation overview must not contain blanks")
    for index, item in enumerate(fill_ins, start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each consolidation fill-in sentence must be an object")
        _require_text(item.get("number"), f"consolidation_sheet.fill_in_sentences[{index}].number")
        sentence = str(item.get("sentence") or "")
        if _blank_count(sentence) != 1:
            raise ScaffoldingError("each consolidation fill-in sentence must contain exactly one blank marker")
        if len(sentence) > 240:
            raise ScaffoldingError("consolidation fill-in sentences must stay short enough for print use")
    ...
    for index, item in enumerate(diagrams, start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each consolidation diagram task must be an object")
        ...
        if len(elements) < 2:
            raise ScaffoldingError("each consolidation diagram task must include at least two required elements")
        for element in elements:
...
def _validate_v3_exam_bridge(exam_bridge: dict[str, Any]) -> None:
    ...
    for index, item in enumerate(_coerce_list(exam_bridge.get("course_connections")), start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each exam bridge course connection must be an object")
    ...
    for index, item in enumerate(_coerce_list(exam_bridge.get("comparison_targets")), start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each exam bridge comparison target must be an object")
    ...
    for index, item in enumerate(_coerce_list(exam_bridge.get("exam_moves")), start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each exam bridge move must be an object")
    ...
    for index, item in enumerate(_coerce_list(exam_bridge.get("misunderstanding_traps")), start=1):
        if not isinstance(item, dict):
            raise ScaffoldingError("each exam bridge misunderstanding trap must be an object")
...
def build_scaffold_for_source(
    *,
    ...
    if json_path.exists() and not force:
        if rerender_existing:
            ...
            if _artifact_schema_version(artifact) >= SCHEMA_VERSION:
                ...
                artifact["scaffolds"] = validate_scaffold_payload(
                    normalize_scaffold_payload(artifact.get("scaffolds", {}))
                )
            else:
    ...
    response = call_json_generator(
        ...
        model=model,
        system_instruction=system_instruction or scaffold_system_instruction(),
        user_prompt=(user_prompt_builder or scaffold_user_prompt)(
            source=source,
    ...
    scaffolds = validate_scaffold_payload(normalize_scaffold_payload(response))
    artifact = {
        ...
        "generator": {
            ...
            "generation_config": scaffold_generation_config_metadata(),
        },
        "provenance": {
...
def render_scaffold_files(*, artifact: dict[str, Any], output_dir: Path, render_pdf: bool = True) -> dict[str, list[str]]:
    if _artifact_schema_version(artifact) < SCHEMA_VERSION:
        return render_v2_scaffold_files(artifact=artifact, output_dir=output_dir, render_pdf=render_pdf)
    return render_v3_scaffold_files(artifact=artifact, output_dir=output_dir, render_pdf=render_pdf)


def render_v2_scaffold_files(*, artifact: dict[str, Any], output_dir: Path, render_pdf: bool = True) -> dict[str, list[str]]:
...
def render_v3_scaffold_files(*, artifact: dict[str, Any], output_dir: Path, render_pdf: bool = True) -> dict[str, list[str]]:
    scaffolds = artifact.get("scaffolds") if isinstance(artifact.get("scaffolds"), dict) else {}
    markdown_items = [
