...
STUDENT_SYNTHESIS_ARCHIVED_NOTEBOOKLM_VARIANT_DECISIONS = (
    STUDENT_SYNTHESIS_FLASHCARD_ARCHIVE / "notebooklm_variant_promotion_decisions.json"
)
STUDENT_SYNTHESIS_ARCHIVED_NOTEBOOKLM_VARIANT_DECK = (
    STUDENT_SYNTHESIS_FLASHCARD_ARCHIVE / "notebooklm-varianter-personlighedspsykologi.json"
)
STUDENT_SYNTHESIS_ARCHIVED_NOTEBOOKLM_INDEPENDENT_VARIANT_DECISIONS = (
    STUDENT_SYNTHESIS_FLASHCARD_ARCHIVE / "notebooklm_independent_variant_promotion_decisions.json"
)
STUDENT_SYNTHESIS_ARCHIVED_NOTEBOOKLM_INDEPENDENT_VARIANT_DECK = (
    STUDENT_SYNTHESIS_FLASHCARD_ARCHIVE / "notebooklm-uafhaengige-varianter-personlighedspsykologi.json"
)
...
def _failures(repo_root: Path) -> list[str]:
    ...
    if not (repo_root / STUDENT_SYNTHESIS_ARCHIVED_FLASHCARD_DECK).exists():
        failures.append(
            f"Missing archived original matrix/Gemini flashcard deck: {STUDENT_SYNTHESIS_ARCHIVED_FLASHCARD_DECK}"
        )
    if not (repo_root / STUDENT_SYNTHESIS_ARCHIVED_NOTEBOOKLM_VARIANT_DECISIONS).exists():
        failures.append(
            "Missing archived NotebookLM variant promotion decisions: "
            f"{STUDENT_SYNTHESIS_ARCHIVED_NOTEBOOKLM_VARIANT_DECISIONS}"
        )
    if not (repo_root / STUDENT_SYNTHESIS_ARCHIVED_NOTEBOOKLM_VARIANT_DECK).exists():
        failures.append(
            f"Missing archived NotebookLM variant flashcard deck: {STUDENT_SYNTHESIS_ARCHIVED_NOTEBOOKLM_VARIANT_DECK}"
        )
    if not (repo_root / STUDENT_SYNTHESIS_ARCHIVED_NOTEBOOKLM_INDEPENDENT_VARIANT_DECISIONS).exists():
        failures.append(
            "Missing archived independent NotebookLM variant promotion decisions: "
            f"{STUDENT_SYNTHESIS_ARCHIVED_NOTEBOOKLM_INDEPENDENT_VARIANT_DECISIONS}"
        )
    if not (repo_root / STUDENT_SYNTHESIS_ARCHIVED_NOTEBOOKLM_INDEPENDENT_VARIANT_DECK).exists():
        failures.append(
            "Missing archived independent NotebookLM variant flashcard deck: "
            f"{STUDENT_SYNTHESIS_ARCHIVED_NOTEBOOKLM_INDEPENDENT_VARIANT_DECK}"
        )
    ...
    archived_flashcard_deck = repo_root / STUDENT_SYNTHESIS_ARCHIVED_FLASHCARD_DECK
    if archived_flashcard_deck.exists() and exam_theory_matrix_payload:
        try:
            deck_payload = _load_json(archived_flashcard_deck)
            ...
            failures.append(
                f"Archived original matrix flashcard deck is invalid in {STUDENT_SYNTHESIS_ARCHIVED_FLASHCARD_DECK}: {exc}"
            )
        else:
            source_file = str(deck_payload.get("source_file") or "").strip()
            if source_file != str(STUDENT_SYNTHESIS_EXAM_THEORY_MATRIX):
                failures.append(
                    "Archived original matrix flashcard deck source_file does not point at the exam matrix: "
                    f"{source_file}"
                )
    ...
    if flashcard_registry.exists() and full_notebooklm_deck.exists():
        try:
            ...
            full_deck_payload = _load_json(full_notebooklm_deck)
            validate_variant_deck(full_deck_payload, expected_deck_slug=FULL_NOTEBOOKLM_DECK_SLUG)
        except (NotebookLMVariantFlashcardError, json.JSONDecodeError) as exc:
            failures.append(
                f"Student synthesis full NotebookLM flashcards are invalid in "
                f"{STUDENT_SYNTHESIS_FULL_NOTEBOOKLM_DECK}: {exc}"
            )
        else:
            if not isinstance(registry_payload, dict) or registry_payload.get("version") != 1:
                failures.append(
                    f"Student synthesis flashcard registry must be a version 1 object: "
            ...
            if isinstance(registry_payload, dict) and registry_payload.get("subject_slug") != "personlighedspsykologi":
                failures.append("Student synthesis flashcard registry subject_slug is not personlighedspsykologi")
            ...
            if not isinstance(decks, list):
                ...
                if len(decks) != 1:
                    failures.append("Student synthesis flashcard registry must contain exactly one live deck")
                registry_entry = decks[0] if len(decks) == 1 and isinstance(decks[0], dict) else None
                if registry_entry is None:
                    failures.append("Student synthesis flashcard registry live deck entry is invalid")
                else:
                    if str(registry_entry.get("deck_slug") or "").strip() != FULL_NOTEBOOKLM_DECK_SLUG:
                        failures.append(
                            f"Student synthesis flashcard registry live deck must be {FULL_NOTEBOOKLM_DECK_SLUG}"
                        )
                    expected_path = str(STUDENT_SYNTHESIS_FULL_NOTEBOOKLM_DECK)
                    actual_path = str(registry_entry.get("artifact_path") or "").strip()
                    if actual_path != expected_path:
                        failures.append(
                            "Full NotebookLM flashcard registry artifact_path mismatch: "
                            f"{actual_path} != {expected_path}"
                        )
                    if int(registry_entry.get("card_count") or 0) != int(full_deck_payload.get("card_count") or 0):
                        failures.append(
                            "Full NotebookLM flashcard registry card_count does not match deck artifact"
                        )
    ...
    archived_variant_decks_to_validate = [
        (
            ...
            STUDENT_SYNTHESIS_ARCHIVED_NOTEBOOKLM_VARIANT_DECK,
            "archived NotebookLM variant",
        ),
        (
            "notebooklm-uafhaengige-varianter-personlighedspsykologi",
            STUDENT_SYNTHESIS_ARCHIVED_NOTEBOOKLM_INDEPENDENT_VARIANT_DECK,
            "archived independent NotebookLM variant",
        ),
    ]
    for expected_deck_slug, deck_relative, label in archived_variant_decks_to_validate:
        variant_deck = repo_root / deck_relative
        if not variant_deck.exists():
            continue
        try:
            validate_variant_deck(_load_json(variant_deck), expected_deck_slug=expected_deck_slug)
        except (NotebookLMVariantFlashcardError, json.JSONDecodeError) as exc:
            failures.append(f"Student synthesis {label} flashcards are invalid in {deck_relative}: {exc}")
