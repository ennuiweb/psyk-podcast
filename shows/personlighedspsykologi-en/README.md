...
Learning-material ledger note: after prompt or Course Understanding changes, check `learning_material_regeneration_registry.json` to see which podcasts, printouts, quizzes, and slides have actually been regenerated or surfaced and when. The queue syncs it automatically after metadata rebuild; manually refresh it with `./.venv/bin/python scripts/sync_personlighedspsykologi_learning_material_registry.py`. Human-facing setup labels now default from `prompt_versions.json`, with env vars kept only as explicit overrides. For lecture-scoped queue runs, podcast entries also get an automatic `prompt_system_label` / `prompt_system_fingerprint`, and if no manual or configured `podcast_setup_version` is supplied the ledger reuses that auto label as the podcast `setup_version`.
