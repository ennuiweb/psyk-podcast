...
Slide short-form note: short-form generation is intentionally limited to all readings plus lecture slides (`short.apply_to: "readings_and_lecture_slides"`; legacy `brief.apply_to` is still accepted). Exercise slides keep their full podcast variants but do not get `Kort podcast` entries under the shared `Forelæsningsslides` label.
