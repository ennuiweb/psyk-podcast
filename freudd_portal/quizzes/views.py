...
def leaderboard_profile_view(request: HttpRequest) -> HttpResponse:
    ...
    try:
        ...
        messages.error(request, "; ".join(exc.messages) if exc.messages else "Kunne ikke opdatere quiz cup-profil.")
    else:
        if is_public:
            messages.success(request, "Din quiz cup-profil er nu offentlig.")
        else:
            messages.info(request, "Din quiz cup-profil er nu privat.")
