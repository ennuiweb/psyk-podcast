...
- Queue core note, 2026-04-29: `notebooklm_queue/` now covers durable queue storage, adapter-based discovery for `bioneuro` and `personlighedspsykologi-en`, dry-run planning, and real queue-owned generate/download execution with persisted run manifests via `scripts/notebooklm_queue.py`. Successful jobs currently stop at `awaiting_publish`; publish orchestration is still pending.
