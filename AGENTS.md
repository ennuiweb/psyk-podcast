...
Freudd remote deploy runbook, verified 2026-06-01:
...
Fallback if the fast-forward merge fails due to dirty server repository state (only with explicit approval):
...
- Expected: `login=200`, `settings=302` (anonymous redirect to login), `public_flashcards=200`, and `public_person_flashcards=200` when checking shared flashcard/personlighedspsykologi changes. `/progress` is a legacy redirect to `/settings` and should return `301`.
