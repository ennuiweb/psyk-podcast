...
from .notebook_reclaim_safety import (
    notebook_payload,
    notebook_sort_key,
        ...
        async with client_context as client:
            ...
            for candidate in sorted(owned, key=notebook_sort_key):
                if len(deleted) >= deletion_budget:
                    break
                blocker = await reclaim_blocker_for_notebook(
                    client=client,
                    notebook_id=str(getattr(candidate, "id", "") or ""),
                    request_log_roots=request_log_roots,
                )
                if blocker:
                    skipped.append(notebook_payload(candidate, reason=blocker))
                    continue
                payload = notebook_payload(candidate)
                if not dry_run:
