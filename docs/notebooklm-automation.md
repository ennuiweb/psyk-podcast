...
- queue execution now applies failure-mode-aware queue-level retry backoff for repeated NotebookLM cooldown, rate-limit, and transient RPC failures: transient failures keep short retries, while rate-limit/profile-cooldown failures use longer hourly backoff so long-running show backlogs stop hammering the same lectures every flat `15` minutes forever
