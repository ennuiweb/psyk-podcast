"""Printable reading printout generation for Personlighedspsykologi."""
...
def printout_response_schema() -> dict[str, Any]:
    quote_target = {
...
def validate_printout_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for item in fill_ins:
        ...
        if "____" not in sentence and "___" not in sentence:
            raise PrintoutError("each cloze sentence must contain a blank marker")
    return payload
...
def build_printout_for_source(
    *,
    ...
    response = call_json_generator(
        ...
        model=model,
        system_instruction=printout_system_instruction(),
        user_prompt=printout_user_prompt(
            source=source,
        ...
        response_json_schema=printout_response_schema(),
    )
    printouts = validate_printout_payload(response)
    artifact = {
...
def render_printout_files(*, artifact: dict[str, Any], output_dir: Path, render_pdf: bool = True) -> dict[str, list[str]]:
    printouts = artifact.get("printouts") if isinstance(artifact.get("printouts"), dict) else {}
    if not printouts:
        printouts = artifact.get("scaffolds") if isinstance(artifact.get("scaffolds"), dict) else {}
    markdown_items = [
        ("01-abridged-guide", render_abridged_markdown(artifact, printouts.get("abridged_guide", {}))),
        ("02-unit-test-suite", render_unit_test_markdown(artifact, printouts.get("unit_test_suite", {}))),
        ("03-cloze-scaffold", render_cloze_markdown(artifact, printouts.get("cloze_scaffold", {}))),
    ]
...
# Legacy compatibility aliases. Keep old import paths working while the repo
# shifts to "printouts" as the canonical term.
ScaffoldingError = PrintoutError
scaffold_response_schema = printout_response_schema
