...
- `scripts/sync_notebooklm_profiles_to_hetzner.py` is the canonical helper for rebuilding the Hetzner profile bundle. A no-argument run is bundle-only and leaves remote storage-state files untouched; real auth uploads require explicit `--profile ...` or `--upload-all`, with backups and newer-remote overwrite protection.
...
- queue execution now applies failure-mode-aware queue-level retry backoff for repeated NotebookLM cooldown, rate-limit, and transient RPC failures: transient failures keep short retries, while rate-limit/profile-cooldown failures and per-profile generator cooldowns use longer hourly backoff so long-running show backlogs stop hammering the same lectures every flat `15` minutes forever
- Personlighedspsykologi queue generation now fails closed when every configured rotation profile is excluded/cooling inside one lecture run; the wrapper stops before the next NotebookLM call and the shared generator exposes `--fail-if-all-profiles-excluded` for automation-safe strictness
- queue execution now checks active NotebookLM profile capacity before claiming generation work, reports it through `profile-status`, treats expired profile validation as an automatic wait for the refresh timer, and uses a global NotebookLM execution lock so concurrent show timers cannot burn the shared profile pool in parallel
...
./.venv/bin/python scripts/notebooklm_queue.py profile-status
```
