...
- Queue core note, 2026-04-29: `notebooklm_queue/` now covers durable queue storage, adapter-based discovery for `bioneuro` and `personlighedspsykologi-en`, dry-run planning, real queue-owned generate/download execution with persisted run manifests, and publish-bundle preparation with durable publish manifests via `scripts/notebooklm_queue.py`. Successful jobs can now advance from `awaiting_publish` to `approved_for_publish`; object upload and metadata publication are still pending.
