...
- For `personlighedspsykologi-en`, audio-only bundles also bypass the manual-summary and slide-brief portal gates, so the queue can push the podcast feed and inventory immediately while portal-specific sidecars wait for non-audio artifacts.
