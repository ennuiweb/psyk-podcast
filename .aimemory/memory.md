...
- Queue core note, 2026-04-29: the first `notebooklm_queue/` implementation now exists as a durable control-plane foundation with deterministic lecture-scoped job IDs, atomic JSON storage, show/global indexes, advisory show locks, transition history, adapter-based discovery for `bioneuro` and `personlighedspsykologi-en`, and a CLI wrapper at `scripts/notebooklm_queue.py`. Queue-owned generation execution and publish orchestration are still pending later phases.
