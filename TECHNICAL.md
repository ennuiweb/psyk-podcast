...
- Question timer: default `30s` per question (`FREUDD_QUIZ_QUESTION_TIME_LIMIT_SECONDS`), auto-timeout marks wrong + advances; when answered, header label switches to earned points for that question.
- Quiz wrapper option order: displayed options are deterministically reordered per question to reduce first-position answer bias, while persisted user answers keep raw option indices so backend scoring remains aligned with source quiz payload.
- Partial quiz progress persistence: disabled in wrapper flow; users always restart from question 1 unless they already completed the quiz.
...
- Quiz Cup scoring: per-quiz point uses correctness + speed bonus (correctness weighted highest), aggregated per subject in active half-year season.
...
- When an RSS title does not expose a lecture key, manifest build can still place the podcast by resolving the RSS item quiz link back to an already-mapped quiz asset.
...
2. Push as usual; the hook runs steps in this order:
   - optional quiz extraction (`extract_quiz_html_to_json.py`) when enabled
...
- pre-push defaults:
  ...
  - `personlighedspsykologi`: mirror without `--delete`
  - set `PERSONLIGHEDSPSYKOLOGI_MIRROR_DELETE_ON_PUSH=1` only for deliberate cleanup pushes when the local output tree is known-complete enough for destructive sync
...
Pre-push environment controls (`personlighedspsykologi`):
...
### Legacy Bioneuro audio mirror (manual utility)
If needed, you can still mirror local Bioneuro audio files from OneDrive into the Google Drive mounted destination used by the `bioneuro` show:
